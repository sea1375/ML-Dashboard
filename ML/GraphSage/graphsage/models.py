...
#added by Amine
class SampleAndAggregatePlus(GeneralizedModel):
    """
    Base implementation of unsupervised GraphSAGE
    """

    def __init__(self, placeholders, features, adjs, degrees,
            layer_infos, concat=True, aggregator_type="mean", 
            model_size="small", identity_dim=0,
            **kwargs):
        '''
        Args:
            ...
            - identity_dim: Set to positive int to use identity features (slow and cannot generalize, but better accuracy)
        '''
        super(SampleAndAggregatePlus, self).__init__(**kwargs)
        if aggregator_type == "mean":
            self.aggregator_cls = MeanAggregator
        elif aggregator_type == "seq":
            self.aggregator_cls = SeqAggregator
        ...
        #for item in self.ids:		
            #self.inputs1[item] = placeholders["batch1"]
            #self.inputs2[item] = placeholders["batch2"]
        self.inputs1 = placeholders["batch1"]
        self.inputs2 = placeholders["batch2"]			
        self.model_size = model_size
        self.adj_info = adjs
        ...
        #self.dims.extend([layer_infos[i].output_dim for i in range(len(layer_infos))])
        for item in features.keys():
            self.dims[item] = [(0 if features is None else features[item].shape[1]) + identity_dim[item]]		
    ...
    def sample(self, inputs, layer_infos, sampler, batch_size=None):
        """ Sample neighbors to be the supportive fields for multi-layer convolutions.
    ...
    def aggregate(self, samples, input_features, dims, num_samples, support_sizes, batch_size=None,
        ...
        #hidden=[]		
        #for node_samples in samples:		
            #hidden.append(tf.gather(input_features,node_samples))
        #print(len(hidden))
        #print(len(hidden1))		
        #input()		
        new_agg = aggregators is None
    ...
    def _build(self):
        ...
        for item in self.ids:
            ...
            self.neg_samples, _, _ = (tf.nn.fixed_unigram_candidate_sampler(
               ...
               num_sampled=FLAGS.neg_sample_size,
               unique=False,
               range_max=len(self.degrees[item]),
               distortion=0.75,
               unigrams=self.degrees[item].tolist()))

           
            # perform "convolution"
            #samples1, support_sizes1 = self.sample(self.inputs1[item], self.layer_infos)
            #samples2, support_sizes2 = self.sample(self.inputs2[item], self.layer_infos)
            samples1, support_sizes1 = self.sample(self.inputs1, self.layer_infos)
            samples2, support_sizes2 = self.sample(self.inputs2, self.layer_infos)			
            num_samples = [layer_info.num_samples for layer_info in self.layer_infos]
            ...
            self.outputs1, self.aggregators = self.aggregate(samples1, [self.features[item]], self.dims[item], num_samples,
                support_sizes1, concat=self.concat, model_size=self.model_size)				
            #self.outputs2[item], _ = self.aggregate(samples2, [self.features[item]], self.dims[item], num_samples,
                #support_sizes2, aggregators=self.aggregators[item], concat=self.concat,
                #model_size=self.model_size)
            self.outputs2, _ = self.aggregate(samples2, [self.features[item]], self.dims[item], num_samples,
                support_sizes2, aggregators=self.aggregators[item], concat=self.concat,
                model_size=self.model_size)
            neg_samples, neg_support_sizes = self.sample(self.neg_samples, self.layer_infos,
                 FLAGS.neg_sample_size)
            #self.neg_outputs[item], _ = self.aggregate(neg_samples, [self.features[item]], self.dims[item], num_samples,
                #neg_support_sizes, batch_size=FLAGS.neg_sample_size, aggregators=self.aggregators[item],
                #concat=self.concat, model_size=self.model_size)
            self.neg_outputs, _ = self.aggregate(neg_samples, [self.features[item]], self.dims[item], num_samples,
                neg_support_sizes, batch_size=FLAGS.neg_sample_size, aggregators=self.aggregators,
                concat=self.concat, model_size=self.model_size)
            dim_mult = 2 if self.concat else 1
            #self.link_pred_layer [item]= BipartiteEdgePredLayer(dim_mult*self.dims[item][-1],
                #dim_mult*self.dims[item][-1], self.placeholders, act=tf.nn.sigmoid, 
                #bilinear_weights=False,
                #name='edge_predict')
            self.link_pred_layer = BipartiteEdgePredLayer(dim_mult*self.dims[item][-1],
                dim_mult*self.dims[item][-1], self.placeholders, act=tf.nn.sigmoid, 
                bilinear_weights=False,
                name='edge_predict')
            #self.outputs1[item] = tf.nn.l2_normalize(self.outputs1[item], 1)
            #self.outputs2[item] = tf.nn.l2_normalize(self.outputs2[item], 1)
            #self.neg_outputs[item] = tf.nn.l2_normalize(self.neg_outputs[item], 1)
            self.outputs1 = tf.nn.l2_normalize(self.outputs1, 1)
            self.outputs2 = tf.nn.l2_normalize(self.outputs2, 1)
            self.neg_outputs = tf.nn.l2_normalize(self.neg_outputs, 1)
    ...
    # def _loss(self):
        # for item in self.ids:	
            # for aggregator in self.aggregators[item]:
                # for var in aggregator.vars.values():
                    # self.loss += FLAGS.weight_decay * tf.nn.l2_loss(var)
            # self.loss += self.link_pred_layer[item].loss(self.outputs1[item], self.outputs2[item], self.neg_outputs[item]) 
        # tf.summary.scalar('loss', self.loss)
    ...
    # def _accuracy(self):
        # for item in self.ids:	
            # # shape: [batch_size]
            # aff = self.link_pred_layer[item].affinity(self.outputs1[item], self.outputs2[item])
            # # shape : [batch_size x num_neg_samples]
            # self.neg_aff[item] = self.link_pred_layer[item].neg_cost(self.outputs1[item], self.neg_outputs[item])
            # self.neg_aff[item] = tf.reshape(self.neg_aff[item], [self.batch_size, FLAGS.neg_sample_size])
            # _aff = tf.expand_dims(aff, axis=1)
            # self.aff_all[item] = tf.concat(axis=1, values=[self.neg_aff[item], _aff])
            # size = tf.shape(self.aff_all[item])[1]
		
            # _, indices_of_ranks = tf.nn.top_k(self.aff_all[item], k=size)
            # _, self.ranks[item] = tf.nn.top_k(-indices_of_ranks, k=size)
            # mrr = tf.reduce_mean(tf.div(1.0, tf.cast(self.ranks[item][:, -1] + 1, tf.float32)))
			
            # self.mrr = tf.reduce_sum(self.mrr,mrr)
        # self.mrr=tf.div(self.mrr,len(self.ids))			
        # tf.summary.scalar('mrr', self.mrr)
